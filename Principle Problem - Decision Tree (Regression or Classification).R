...
library(readxl)
myData <- Ch13_Q13_Data_File <- read_excel("C:/Users/willa/Downloads/Ch13_Q13_Data_File.xlsx")
...
# 1. set variable we trying to predict the outcome of as binary and then partition data into 70:30 train and holdout
...
pruned_tree <- prune(full_tree, cp = 0.0172415) #NOTICE WE INCREASED THE LAST DIGIT BY 1!!!!!!!
...
gains_table
plot(c(0, gains_table$cume.pct.of.total*sum(validationSet$ContinueEdu)) ~ c(0, gains_table$cume.obs), xlab = '# of cases', ylab = "Cumulative", type = "l")
lines(c(0, sum(validationSet$ContinueEdu))~c(0, dim(validationSet)[1]), col="red", lty=2)
barplot(gains_table$mean.resp/mean(validationSet$ContinueEdu), names.arg=gains_table$depth, xlab="Percentile", ylab="Lift", ylim=c(0, 3.0), main="Decile-Wise Lift Chart")
...
myScoreData <- read_excel("C:/Users/willa/Downloads/Ch13_Q13_Score_File.xlsx")
View(myScoreData)
