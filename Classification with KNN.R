...
library(readxl)
myData <- read_excel("C:/Users/willa/Downloads/Ch12_Q3_Data_File.xlsx")
View(myData)

install.packages(c("carat", "gains", "pROC", "klaR"))
library(caret)
library(gains)
library(pROC)
library(klaR)
...
roc_object<- roc(VData$y, KNN_Class_prob[,2])
View(myData)
...
myData1<-scale(myData[,2:5])			# c. data standardization and preparation
myData1<-data.frame(myData1, myData$y)
View(myData1)
colnames(myData1)[5]<-'Enroll'
...
myScoreData <- read_excel("C:/Users/willa/Downloads/Ch12_Q3_Score_File.xlsx")
